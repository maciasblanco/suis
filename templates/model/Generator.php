...
class Generator extends \yii\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass', 'descriptionColumn'], 'filter', 'filter' => 'trim'],
            ...
            [['descriptionColumn'], 'validateColumnName', 'skipOnEmpty' => true],
        ]);
    }
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'descriptionColumn' => 'The column to use for override the "__toString" method',
        ]);
    }
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            if (!in_array($column->name, $columsForNull)) {
                $columsForNull[] = $column->name;
            }
        ...
        if (!empty($columsForNull)) {
            $rules[] = "[['" . implode("', '", $columsForNull) . "'], 'default', 'value' => null]";
        }
     ...
     */
    public function validateColumnName()
    {
        if (empty($this->tableName)) {
            $this->addError('descriptionColumn', "You need to specify a table.");
        }

        $db = $this->getDbConnection();
        $table = $db->getTableSchema($this->tableName, true);
        $exists = false;

        if ($table != null) {
            foreach ($table->columns as $col) {
                if ($col->name == $this->descriptionColumn) {
                    $exists = true;
                    break;
                }
            }
        ...
        if (!$exists) {
            $this->addError('descriptionColumn', 'This column doesn\'t exists.');
        }
