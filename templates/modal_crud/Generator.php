...
namespace app\templates\modal_crud;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['enableI18N', 'enablePjax', 'useModals'], 'boolean'],
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($ref = $this->getForeignKey($column->name)) {
            $class = Inflector::camelize($ref['table']);
            $col = $ref['attribute'];
            return "\$form->field(\$model, '$attribute')->dropDownList("
                . "ArrayHelper::map({$class}::find()->all(), '{$col}', '{$col}')"
                . ", ['prompt' => ''])";
        }
     ...
     * @return array|boolean An array with the referenced table and referenced column
     * In case the column isn't an foreign key returns false
     */
    public function getForeignKey($columnName)
    {
        $tableSchema = $this->getTableSchema();
        if (!empty($tableSchema->foreignKeys)) {
            foreach ($tableSchema->foreignKeys as $fkName => $fkData) {
                $referencedTable = $fkData[0];
                unset($fkData[0]);

                $fkColumns = array_keys($fkData);

                if (!empty($fkColumns)) {
                    foreach ($fkColumns as $fkCol) {
                        if ($fkCol == $columnName) {
                            return ['table' => $referencedTable, 'attribute' => $fkData[$columnName]];
                        }
                    }
                }
            }
